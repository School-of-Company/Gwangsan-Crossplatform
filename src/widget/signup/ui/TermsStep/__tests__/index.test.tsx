...
describe('TermsStep — 렌더링', () => {
  it('타이틀, 설명, 약관 내용, 동의 버튼을 렌더링한다', () => {
    const { getByText } = render(<TermsStep />);

    expect(getByText('이용약관')).toBeTruthy();
    expect(getByText('서비스 이용을 위해 약관에 동의해주세요')).toBeTruthy();
    expect(getByText(/부적절한 콘텐츠 금지/)).toBeTruthy();
    expect(getByText('동의하고 계속')).toBeTruthy();
    expect(getByText('위 이용약관 및 무관용 정책에 동의합니다')).toBeTruthy();
  });
});

describe('TermsStep — 약관 동의', () => {
  it('체크박스를 누르기 전에는 다음 버튼이 비활성화 상태다', () => {
    const { getByTestId } = render(<TermsStep />);

    expect(getByTestId('next-button').props.accessibilityState?.disabled).toBe(true);
  });

  it('체크박스를 누르면 다음 버튼이 활성화되고, 다시 누르면 비활성화된다', () => {
    const { getByTestId, getByText } = render(<TermsStep />);

    fireEvent.press(getByText('위 이용약관 및 무관용 정책에 동의합니다'));
...
describe('TermsStep — 다음 단계로 이동', () => {
  it('동의 후 다음 클릭 시 nextStep이 호출된다', () => {
    const { getByTestId, getByText } = render(<TermsStep />);
