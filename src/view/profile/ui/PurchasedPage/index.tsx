...
export default function PurchasedPageView() {
  ...
  const purchasedPosts = Array.isArray(postsData)
  ...
  return (
    <SafeAreaView className="flex-1 bg-white" edges={['top', 'left', 'right']}>
      <Header
        headerTitle={isMe ? '구매내역' : `${profileData?.nickname ?? ''}님의 구매 내역`}
        showBackButton
      />
      <ScrollView className="flex-1 px-6 pt-6" showsVerticalScrollIndicator={false}>
        {purchasedPosts.length > 0 ? (
          ...
          <Text className="pt-20 text-center text-gray-500">구매 내역이 없습니다.</Text>
        )}
      </ScrollView>
    </SafeAreaView>
  );
