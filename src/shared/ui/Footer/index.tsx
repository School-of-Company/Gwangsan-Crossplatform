...
// iOS 시트 프레젠테이션에서 쓰이는 곡선 (BottomSheetModalWrapper와 동일)
const APPLE_SHEET_EASING = Easing.bezier(0.32, 0.72, 0, 1);
const HIDE_DURATION = 380; // 모달이 올라오는 시간과 맞춤
const SHOW_DURATION = 300; // 모달이 내려가는 시간과 맞춤
...
function useFooterHideAnimation(isHidden: boolean, footerHeight: number) {
  ...
  useEffect(() => {
    Animated.timing(translateY, {
      toValue: isHidden ? footerHeight || FALLBACK_HIDE_OFFSET : 0,
      duration: isHidden ? HIDE_DURATION : SHOW_DURATION,
      easing: APPLE_SHEET_EASING,
      useNativeDriver: true,
    }).start();
  }, [isHidden, footerHeight, translateY]);
...
export function Footer({ state, navigation, insets, onWritePress }: FooterProps) {
  ...
  const hideTranslateY = useFooterHideAnimation(isHidden, footerHeight);
  ...
  return (
    <Animated.View
      className="relative bottom-0 w-full flex-row border-t border-gray-200 bg-white px-2 pt-1"
      style={{
      ...
      <TouchableOpacity
        ...
        <Text className={activeRouteName === 'main' ? 'text-[#8FC31D]' : 'text-gray-500'}>홈</Text>
      </TouchableOpacity>
      <TouchableOpacity
        className="flex-1 items-center justify-center py-2"
        hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
        ...
        <Animated.View className="relative" style={{ transform: [{ scale: chattingScale.scale }] }}>
          <Ionicons
