...
// ProgressBar는 레이아웃이 잡힐 때 `sliderRef.current.measureInWindow(...)`로 트랙의
// 화면 좌표를 한 번만 측정해 캐시한다. jest-expo의 react-test-renderer 설정에서는
// `createNodeMock`이 (버전 조합 문제로) 호출되지 않아 ref가 항상 null로 남는다.
// 대신 'react-native'의 View export를 (모듈 전체를 교체하는 게 아니라, 이미 완전히
// 초기화된 실제 모듈 객체의 View 프로퍼티만) `measureInWindow`를 노출하는 forwardRef
// 래퍼로 재정의해 실제 ref 부착 경로를 검증한다. `{...RN}`처럼 모듈 전체를
// 스프레드하면 TurboModule의 지연 getter가 즉시 평가되며 깨지므로, 반드시 실제 모듈
// 객체를 그대로 유지한 채 View 프로퍼티만 교체한다.
const mockMeasureInWindowFn = jest.fn((callback: (x: number, y: number) => void) => callback(0, 0));
  ...
  const ViewWithMeasure = ReactActual.forwardRef((props: unknown, ref: unknown) => {
    ReactActual.useImperativeHandle(ref, () => ({
      measureInWindow: (callback: (x: number, y: number) => void) =>
        mockMeasureInWindowFn(callback),
    }));
...
describe('ProgressBar', () => {
  ...
  describe('PanResponder 콜백 (config 캡처)', () => {
    afterEach(() => {
      ...
      capturedConfig = null;
      mockMeasureInWindowFn.mockClear();
    });
    ...
    it('부모 ScrollView가 드래그를 가로채지 못하도록 캡처/종료 협상을 막는다', () => {
      withCapturedPanResponderConfig();
      render(<ProgressBar value={50} onChange={jest.fn()} />);

      expect(capturedConfig.onMoveShouldSetPanResponderCapture()).toBe(true);
    ...
    it('드래그 중에는 measureInWindow를 다시 호출하지 않는다', () => {
      ...
      act(() => {
        capturedConfig.onPanResponderGrant({ nativeEvent: { pageX: 150 } });
        capturedConfig.onPanResponderMove({ nativeEvent: { pageX: 160 } });
        capturedConfig.onPanResponderMove({ nativeEvent: { pageX: 170 } });
      });

      expect(mockMeasureInWindowFn).toHaveBeenCalledTimes(measureCallsAfterLayout);
    });

    it('레이아웃이 잡히기 전에는 onChange를 호출하지 않는다', () => {
      withCapturedPanResponderConfig();
      const onChange = jest.fn();
    ...
    it('언마운트되어 ref가 정리된 후에는 onPanResponderGrant/Move가 onChange를 호출하지 않는다', () => {
      ...
      // `if (!sliderRef.current) return;` 가드가 걸려 아무 일도 일어나지 않는다.
      expect(() =>
        act(() => {
          capturedConfig.onPanResponderGrant({ nativeEvent: { pageX: 150 } });
          capturedConfig.onPanResponderMove({ nativeEvent: { pageX: 80 } });
