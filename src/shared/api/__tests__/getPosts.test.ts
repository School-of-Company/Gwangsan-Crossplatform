...
describe('getPosts', () => {
  ...
  describe('쿼리 파라미터', () => {
    it('type 파라미터를 URL에 포함한다', async () => {
      ...
      await getPosts('OBJECT' as ProductType);

      expect(mockInstance.get).toHaveBeenCalledWith(expect.stringContaining('type=OBJECT'));
    });

    it('mode 파라미터를 URL에 포함한다', async () => {
      mockInstance.get.mockResolvedValue({ data: [] });

      await getPosts(undefined, 'GIVER' as ModeType);

      expect(mockInstance.get).toHaveBeenCalledWith(expect.stringContaining('mode=GIVER'));
    });

    it('type과 mode 모두 전달하면 둘 다 포함한다', async () => {
      mockInstance.get.mockResolvedValue({ data: [] });

      await getPosts('SERVICE' as ProductType, 'RECEIVER' as ModeType);

      const calledUrl = (mockInstance.get as jest.Mock).mock.calls[0][0] as string;
      expect(calledUrl).toContain('type=SERVICE');
      expect(calledUrl).toContain('mode=RECEIVER');
    });
