...
describe('useReport', () => {
  describe('초기 상태', () => {
    it('reportType이 null이다', () => {
      const { result } = renderHookWithProviders(() =>
        useReport({ productId: 1, memberId: 1 })
      );
  ...
  describe('handleSubmit', () => {
    it('productId가 있으면 PRODUCT 타입으로 신고한다', async () => {
      ...
      const { result } = renderHookWithProviders(() =>
        useReport({ productId: 5, memberId: 42 })
      );
    ...
    it('이미지 ID가 있으면 payload에 포함된다', async () => {
      ...
      expect(mockReport).toHaveBeenCalledWith(
        expect.objectContaining({ imageIds: [10, 20] })
      );
    });
    ...
    it('신고 성공 시 onSuccess 콜백을 호출한다', async () => {
      ...
      const { result } = renderHookWithProviders(() =>
        useReport({ memberId: 42, onSuccess })
      );
